...
| ``FATAL: run `x` is already being trained by another process`` | two terminals, one forgotten `NAME=` | give the second one a name of its own. If the other job really is dead, `rm modelstore/x/.training.lock` -- the message prints the host, pid and SLURM job id that took it |
